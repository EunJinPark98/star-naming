...
(function () {
   ...
   * 지은 이름을 그림 한 장으로 그린다.
   *
   * 카카오 카드에 들어가는 그림은 밖에서 받아갈 수 있는 주소라야 해서
   * 이 그림을 바로 얹지는 못한다. 대신 이 그림을 그대로 내려받거나
   * (휴대폰에서는) 사진째로 공유할 수 있게 해 준다.
   */
  const CARD = 1080;
  ...
  function drawStars(ctx, w, h, n) {
    for (let i = 0; i < n; i++) {
      ...
      ctx.fillStyle =
        Math.random() < 0.3
          ? "rgba(245,197,66," + (Math.random() * 0.5 + 0.2).toFixed(2) + ")"
          : "rgba(255,255,255," + (Math.random() * 0.4 + 0.15).toFixed(2) + ")";
      ctx.fill();
    }
  }
  ...
  async function ensureFonts() {
    ...
    try {
      await Promise.all([
        document.fonts.load('700 132px "Gowun Batang"'),
        document.fonts.load('700 54px "Gowun Batang"'),
        document.fonts.load('700 46px "Gowun Batang"'),
        document.fonts.load('400 32px "Gowun Dodum"'),
      ]);
  ...
  async function renderResultImage() {
    ...
    /* 머리말 */
    const HEAD = '400 32px "Gowun Dodum", "Apple SD Gothic Neo", sans-serif';
    const BADGE = '400 28px "Gowun Dodum", "Apple SD Gothic Neo", sans-serif';
    const NAME = '700 132px "Gowun Batang", "Apple SD Gothic Neo", serif';
    const HANJA = '700 54px "Gowun Batang", "Apple SD Gothic Neo", serif';
    const CHARS = '400 32px "Gowun Dodum", "Apple SD Gothic Neo", sans-serif';
    const MEAN = '700 46px "Gowun Batang", "Apple SD Gothic Neo", serif';

    ctx.font = HEAD;
    ctx.fillStyle = "rgba(245,197,66,0.9)";
    ctx.fillText("✦  별 별  작 명 소  ✦", W / 2, 128);

    const isPure = o.script === "hangul";

    /* 담을 줄을 먼저 모아 두고, 머리말과 꼬리말 사이 한가운데에 놓는다.
    ...
    /* 꼬리말 */
    ctx.font = '400 28px "Gowun Dodum", "Apple SD Gothic Neo", sans-serif';
  ...
  $("imageBtn").addEventListener("click", async () => {
    ...
    try {
      ...
      if (!blob) return;
      const name = "별별작명소_" + (current ? current.result.full : "이름") + ".png";
      const file = new File([blob], name, { type: "image/png" });

      /* 휴대폰에서는 사진째로 공유창을 띄운다 */
      ...
      console.warn("[별별 작명소] 이미지를 만들지 못했어요:", e);
