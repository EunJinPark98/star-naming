...
`x: 1` 은 어른 이름에는 흔해도 요즘 아이 이름으로는 잘 쓰지 않는 한자에
붙입니다. 부모님이 한자를 고르는 자리에는 그대로 보이지만, 새 이름을 지을
때는 뽑지 않습니다. 부모님이 직접 고른 글자는 물려받을 때 그대로 씁니다.
음절 자체가 부모님 세대의 것이면 `o: 1` 로 음절째 빼 둡니다.
